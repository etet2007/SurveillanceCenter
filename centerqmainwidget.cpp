...
CenterQMainWidget::CenterQMainWidget(QWidget *parent) : QWidget(parent)
{
    ...
    m_serverIp=settings.value("serverHostUrl","http://192.168.153.174:8001/").toString();


    //创建myQGraphicsScene，父控件释放后会自动释放
    ...
    myQGraphicsView->backgroundImageUrl=settings.value("backgroundImageUrl","para.bmp").toString();
    //读取背景图片
    myQGraphicsView->readBackgroundPic();
    //初始化Url
    cuttingnodePutUrl=m_serverIp+"cuttingnode/";
    recombinationnodePutUrl=m_serverIp+"recombinationnode/";
    ...
    //右方工具栏 mainGroupBox 包含所有东西
    QGroupBox *mainGroupBox = new QGroupBox(this);
    mainGroupBox->setFixedWidth(250);
    mainGroupBox->setTitle("工具栏");

    //场景控制GroupBox
    QGroupBox *sceneCtrlGroupBox = new QGroupBox (mainGroupBox);
    sceneCtrlGroupBox->setTitle("场景控制");

    //旋转控件GroupBox
    QGroupBox *rotateGroupBox = new QGroupBox (sceneCtrlGroupBox);
    ...
    //缩放控件GroupBox
    QGroupBox *scaleGroupBox = new QGroupBox(sceneCtrlGroupBox);
    ...
    //重置按钮
    QPushButton *resetButton = new QPushButton(sceneCtrlGroupBox);
    resetButton->setText(tr("重置变换参数"));
    ...
    //数据获取与显示GroupBox
    QGroupBox *nodeDataGroupBox = new QGroupBox (mainGroupBox);
    nodeDataGroupBox->setTitle("数据获取与显示");

    //查询按钮
    QPushButton *queryButton = new QPushButton(nodeDataGroupBox);
    queryButton->setText(tr("获取节点数据"));
    connect(queryButton, SIGNAL(clicked(bool)), this, SLOT(slot_requestData()));

    //摄像机参数列表
    cameraListWidget = new QListWidget(nodeDataGroupBox);
    ...
    //nodeDataCalcGroupBox
    QGroupBox *nodeDataCalcGroupBox = new QGroupBox (mainGroupBox);
    nodeDataCalcGroupBox->setTitle("参数计算与上传");

    //计算按钮
    QPushButton *calcButton = new QPushButton(nodeDataCalcGroupBox);
    calcButton->setText(tr("生成节点参数"));
    connect(calcButton, SIGNAL(clicked(bool)), this, SLOT(slot_calcTopologicalData()));

    //更新按钮
    QPushButton *uploadDataButton = new QPushButton(nodeDataCalcGroupBox);
    uploadDataButton->setText(tr("上传节点参数"));
    connect(uploadDataButton, SIGNAL(clicked(bool)), this, SLOT(slot_upLoadData()));

    //上传底图按钮
    QPushButton *uploadBackgroundImageButton = new QPushButton(nodeDataCalcGroupBox);
...
//!设置控件的Layout
    ...
    //sceneCtrlGroupBox layout
    QVBoxLayout *sceneCtrlGroupBoxLayout = new QVBoxLayout(sceneCtrlGroupBox);
    sceneCtrlGroupBoxLayout->addWidget(rotateGroupBox);
    sceneCtrlGroupBoxLayout->addWidget(scaleGroupBox);
    sceneCtrlGroupBoxLayout->addWidget(resetButton);
    ...
    QVBoxLayout *nodeDataGroupBoxLayout = new QVBoxLayout(nodeDataGroupBox);
    nodeDataGroupBoxLayout->addWidget(queryButton);
    nodeDataGroupBoxLayout->addWidget(cameraListWidget);

    QVBoxLayout * nodeDataCalcGroupBoxLayout=new QVBoxLayout(nodeDataCalcGroupBox);
    nodeDataCalcGroupBoxLayout->addWidget(uploadBackgroundImageButton);
    nodeDataCalcGroupBoxLayout->addWidget(calcButton);
    nodeDataCalcGroupBoxLayout->addWidget(uploadDataButton);


    //mainGroupBox layout
    QVBoxLayout *mainGroupLayout = new QVBoxLayout(mainGroupBox);
    mainGroupLayout->addWidget(sceneCtrlGroupBox);
    mainGroupLayout->addStretch(1);
//    mainGroupLayout->addWidget(openRegisterDialogButton);//应该转移到menu栏中。
    mainGroupLayout->addWidget(nodeDataGroupBox);
    mainGroupLayout->addStretch(1);
    mainGroupLayout->addWidget(nodeDataCalcGroupBox);
...
//    qDebug()<<areaString;
    QString cuttingNodeSearchUrl=m_serverIp+"cuttingnode/search/"+areaString+"/?format=json";
    QString recombinationNodeGetUrl=m_serverIp+"recombinationnode/?format=json";

    qDebug()<<cuttingNodeSearchUrl;
    qDebug()<<recombinationNodeGetUrl;

    //根据URL生产request对象
    QNetworkRequest & cuttingNodeRequest=QNetworkRequest(QUrl(cuttingNodeSearchUrl));
    QNetworkRequest & recombinationNodeRequest=QNetworkRequest(QUrl(recombinationNodeGetUrl));
...
//    addItems();
    itemSelectedBrush.setStyle(Qt::DiagCrossPattern);
    itemSelectedBrush.setColor(Qt::red);
}
...
void CenterQMainWidget::slot_itemEntered(QListWidgetItem *current)
{
    ...
    if(cuttingNodeEntered!=NULL && cuttingNodeEntered!=cuttingNodeSelected)
    {
        ...
        polygonItemTemp->setBrush(QBrush(Qt::NoBrush));
        }
    }
    ...
    if(cuttingNodeEntered!=NULL&&cuttingNodeEntered!=cuttingNodeSelected)
    {
        ...
        polygonItemTemp->setBrush(itemSelectedBrush);
        }
    }
}
...
void CenterQMainWidget::slot_calcTopologicalData(){
    ...
    for(QListIterator<CuttingNode*> iterator(cuttingNodeList);iterator.hasNext();)
    {
        ...
        for(int i=0;i<myQGraphicsView->verticalLineList.size();i++)
        {
            ...
            if(polygonAreaRectTopLeftX<boundary && boundary<polygonAreaRectBottonRightX)
            { //polygonAreaRectTopLeftX<boundary<polygonAreaRectBottonRightX
...
void CenterQMainWidget::slot_recombinationNodeReplyReadyRead()
{
    ...
    {
        QMessageBox msgBox;
        QString title="RecombinationNode Json解析错误，错误代码：";
        qDebug()<<json_error.error;
        title.append(QString::number(json_error.error));
...
void CenterQMainWidget::slot_getCameraFrame(QListWidgetItem* listWidgetItem)
{
    ...
    {
//        myQGraphicsView->fitInView(cuttingNodeTemp->getPolygonItem(),Qt::KeepAspectRatio);
        myQGraphicsView->centerOn(cuttingNodeTemp->getPolygonItem());
        //Error.
    }else{
        return ;
    }
